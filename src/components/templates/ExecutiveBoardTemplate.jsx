...
const ExecutiveBoardTemplate = ({ markdown, userProfile }) => {
  ...
  return (
    <div className="bg-white mx-auto font-sans text-slate-900 leading-snug">
      <style>{`
          ...
          <div className="text-right text-xs font-['Merriweather_Sans',sans-serif] text-[#cbd5e1] leading-5">
            {contactItems.map((item, i) => (
              <div key={i} className="flex items-center justify-end gap-2">
                ...
                <item.icon size={12} className="text-[#cbd5e1] opacity-70" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-12 font-['Merriweather_Sans',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-lg font-['Roboto_Slab',serif] font-bold text-[#0f172a] mt-8 mb-4 border-b-2 border-[#cbd5e1] pb-1 uppercase tracking-tight"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            a: ({ node, ...props }) => (
              <a
                className="text-[#0f172a] underline decoration-[#94a3b8] hover:decoration-[#0f172a]"
                {...props}
              />
            ),
          }}
