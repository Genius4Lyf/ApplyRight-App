      ...
      <header className="px-10 py-8 bg-[#f2f2f2] border-b-2 border-slate-300">
        ...
        <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs font-bold font-['Roboto',sans-serif] text-slate-800 uppercase tracking-wide">
          {contactItems.map((item, i) => (
            <div
              key={i}
              className="flex items-center gap-2 border-b border-transparent hover:border-[#CC0000] transition-colors"
            >
      ...
      <div className="p-10 font-['Roboto',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-2xl font-['Teko',sans-serif] font-medium text-black mt-8 mb-4 border-l-4 border-[#CC0000] pl-3 uppercase tracking-wide"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-square pl-5 mb-4 text-[10.5pt] leading-6 text-[#1a1a1a] marker:text-[#CC0000]"
                {...props}
              />
            ),
