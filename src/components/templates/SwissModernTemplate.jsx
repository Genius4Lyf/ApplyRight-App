              ...
              {contactItems.length > 0 && (
                <div className="flex flex-col items-start gap-1 text-sm font-bold text-slate-500">
                  {contactItems.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 hover:text-indigo-600 transition-colors cursor-default"
                    >
          ...
          <div className="cv-body">
            <ReactMarkdown
              components={{
                ...
                h2: ({ node, ...props }) => (
                  <h2
                    className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-6 mt-12 border-t-4 border-indigo-600 pt-6 first:mt-0"
                    {...props}
                  />
                ),
                ...
                p: ({ node, ...props }) => (
                  <p
                    className="text-sm font-medium leading-relaxed text-slate-700 mb-4 max-w-prose"
                    {...props}
                  />
                ),
                ...
                ul: ({ node, ...props }) => (
                  <ul
                    className="list-disc mb-6 space-y-2 text-sm font-medium leading-relaxed text-slate-700 ml-4"
                    {...props}
                  />
                ),
                ...
                a: ({ node, ...props }) => (
                  <a
                    className="text-slate-900 font-bold underline decoration-2 decoration-indigo-600"
                    {...props}
                  />
                ),
