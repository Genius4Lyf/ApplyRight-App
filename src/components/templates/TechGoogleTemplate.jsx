...
const TechGoogleTemplate = ({ markdown, userProfile }) => {
  ...
  return (
    <div className="bg-white mx-auto font-sans text-[#202124] leading-relaxed relative overflow-hidden">
      <style>{`
      ...
      <header className="px-10 py-8 pb-4">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            ...
            {roleTitle && <div className="text-lg text-[#5f6368] font-light">{roleTitle}</div>}
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {contactItems.map((item, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#f1f3f4] text-xs font-medium text-[#5f6368] hover:bg-[#e8eaed] transition-colors"
            >
              {/* Icon Logic based on content if possible, simplistic here */}
              {i % 4 === 0 ? (
                <item.icon size={14} className="text-[#4285F4]" />
              ) : i % 4 === 1 ? (
                <item.icon size={14} className="text-[#DB4437]" />
              ) : i % 4 === 2 ? (
                <item.icon size={14} className="text-[#F4B400]" />
              ) : (
      ...
      <div className="px-10 pb-12 font-['Roboto',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-xl font-medium text-[#202124] mt-8 mb-4 flex items-center gap-2"
                {...props}
              >
                {props.children}
              </h2>
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-disc pl-5 mb-4 text-[10.5pt] leading-7 text-[#3c4043] marker:text-[#9aa0a6]"
                {...props}
              />
            ),
