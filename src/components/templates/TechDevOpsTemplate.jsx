      ...
      <div className="p-8 font-['Fira_Code',monospace]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-lg font-bold text-[#569cd6] mt-8 mb-4 border-b border-[#333] pb-1"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-none space-y-2 mb-4 text-xs leading-6 text-[#cccccc]"
                {...props}
              />
            ),
            li: ({ node, ...props }) => (
              <li
                className="pl-4 relative before:content-['>'] before:absolute before:left-0 before:text-[#569cd6]"
                {...props}
              />
            ),
            strong: ({ node, ...props }) => (
            ...
            a: ({ node, ...props }) => <a className="text-[#4ec9b0] hover:underline" {...props} />,
          }}
        >
          {bodyMarkdown}
        </ReactMarkdown>
