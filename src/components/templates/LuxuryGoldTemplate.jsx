...
const LuxuryGoldTemplate = ({ markdown, userProfile }) => {
  ...
  return (
    <div className="bg-white mx-auto font-sans text-slate-900 leading-relaxed shadow-lg">
      <style>{`
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-xs text-slate-400 font-medium tracking-wider uppercase">
            {contactItems.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-1 hover:text-amber-500 transition-colors cursor-default"
              >
      ...
      <div className="px-16 pb-16 font-['Lato',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-xl font-['Playfair_Display',serif] text-slate-900 mt-10 mb-6 flex items-center gap-4"
                {...props}
              >
            ...
            h4: ({ node, ...props }) => (
              <h4
                className="text-xs font-bold text-amber-600 mb-3 uppercase tracking-wide"
                {...props}
              />
            ),
            p: ({ node, ...props }) => (
              <p
                className="mb-4 text-justify font-light text-slate-600 leading-7 text-[10.5pt]"
                {...props}
              />
            ),
            ul: ({ node, ...props }) => (
              <ul
                className="list-none space-y-2 mb-6 text-[10.5pt] font-light text-slate-600"
                {...props}
              />
            ),
            li: ({ node, ...props }) => (
              <li
                className="pl-4 relative before:content-['•'] before:absolute before:left-0 before:text-amber-300"
                {...props}
              />
            ),
            strong: ({ node, ...props }) => (
