...
const LuxuryRoyalTemplate = ({ markdown, userProfile }) => {
  ...
  if (userProfile?.phone)
    contactParts.push({ icon: <Phone size={10} />, value: userProfile.phone });
  ...
  return (
    <div className="bg-slate-900 mx-auto font-['Playfair_Display',serif] text-slate-300 leading-relaxed min-h-[297mm]">
      ...
      <header className="text-center pt-16 pb-6 px-10 bg-slate-900 relative">
        ...
        {roleTitle && (
          <div className="text-sm uppercase tracking-[0.3em] text-slate-400 mb-8 font-sans">
            {roleTitle}
          </div>
        )}
      ...
      <div className="px-12 pb-12 pt-2 font-['Lato',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-2xl font-['Playfair_Display',serif] text-amber-500 mt-6 mb-6 pb-2 border-b border-amber-900/50 text-center italic break-after-avoid"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            h4: ({ node, ...props }) => (
              <h4
                className="text-xs uppercase tracking-widest text-slate-500 mb-4 text-center"
                {...props}
              />
            ),
            p: ({ node, ...props }) => (
            ...
            a: ({ node, ...props }) => (
              <a
                className="text-amber-400 hover:text-amber-300 transition-colors underline decoration-amber-900"
                {...props}
              />
            ),
          }}
