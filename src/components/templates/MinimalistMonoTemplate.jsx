  ...
  return (
    <div className="bg-white mx-auto p-12 font-['JetBrains_Mono',monospace] text-slate-800 leading-relaxed text-[10pt]">
      ...
      <header className="mb-12 border-b border-dashed border-slate-300 pb-8">
        <h1 className="text-2xl font-bold uppercase tracking-tight text-slate-900 mb-2">{name}</h1>
        {roleTitle && (
      ...
      <div className="cv-body space-y-2">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-sm font-bold uppercase bg-slate-100 p-1 pl-2 mb-4 mt-8 select-none"
                {...props}
              >
                {props.children}
              </h2>
            ...
            h3: ({ node, ...props }) => (
              <h3
                className="text-sm font-bold text-slate-900 mt-6 mb-1 underline decoration-slate-300 decoration-dashed underline-offset-4"
                {...props}
              />
            ),
            h4: ({ node, ...props }) => <h4 className="text-xs text-slate-500 mb-3" {...props} />,
            p: ({ node, ...props }) => <p className="mb-3 text-justify" {...props} />,
            ul: ({ node, ...props }) => <ul className="list-none pl-0 mb-4 space-y-2" {...props} />,
            li: ({ node, ...props }) => (
              <li
                className="pl-4 relative before:content-['>'] before:absolute before:left-0 before:text-slate-400 before:font-bold"
                {...props}
              />
            ),
            strong: ({ node, ...props }) => (
