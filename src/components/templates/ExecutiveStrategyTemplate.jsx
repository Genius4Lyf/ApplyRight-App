      ...
      <main className="w-[68%] p-10 font-['Open_Sans',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-lg font-bold text-[#334155] uppercase tracking-wide mb-4 mt-8 flex items-center gap-3 before:content-[''] before:w-1.5 before:h-1.5 before:bg-[#0ea5e9] before:rounded-full"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-none space-y-2 mb-4 text-[10.5pt] leading-6 text-[#475569]"
                {...props}
              />
            ),
            li: ({ node, ...props }) => (
            ...
            a: ({ node, ...props }) => <a className="text-[#0ea5e9] hover:underline" {...props} />,
          }}
        >
          {bodyMarkdown}
        </ReactMarkdown>
