            ...
            {contactParts.length > 0 && (
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-600 font-medium">
                {contactParts.map((part, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 hover:text-indigo-600 transition-colors bg-white px-2 py-1 rounded border border-slate-100 shadow-sm"
                  >
      ...
      <div className="cv-body px-12 pb-12">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-xl font-bold text-slate-900 mb-6 mt-10 flex items-center gap-3 after:content-[''] after:h-[2px] after:flex-1 after:bg-indigo-100"
                {...props}
              >
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-none mb-6 space-y-2 text-sm leading-7 text-slate-600"
                {...props}
              />
            ),
            ...
            li: ({ node, ...props }) => (
              <li
                className="relative pl-5 before:content-['•'] before:absolute before:left-0 before:text-purple-500 before:font-bold"
                {...props}
              />
            ),
