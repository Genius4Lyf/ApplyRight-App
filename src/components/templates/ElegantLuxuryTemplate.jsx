...
const ElegantLuxuryTemplate = ({ markdown, userProfile }) => {
  ...
  try {
    ...
    if (userProfile?.linkedinUrl || userProfile?.linkedin)
      contactItems.push({
        icon: Linkedin,
        value: (userProfile.linkedinUrl || userProfile.linkedin).replace(
          /^https?:\/\/(www\.)?/,
          ''
        ),
      });
        ...
        <header className="text-center mb-8 relative">
          ...
          <div className="inline-block bg-[#fcfbf9] px-10 relative z-10">
            ...
            {contactItems.length > 0 && (
              <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mt-6 text-[11px] font-bold uppercase tracking-widest text-slate-600 px-8">
        ...
        <div className="cv-body font-['Lato',sans-serif]">
          <ReactMarkdown
            components={{
              ...
              h2: ({ node, ...props }) => (
                <h2
                  className="text-2xl font-['Playfair_Display',serif] text-slate-900 mb-6 mt-8 flex flex-col items-center gap-2"
                  {...props}
                >
                  {props.children}
              ...
              h4: ({ node, ...props }) => (
                <h4
                  className="text-xs font-bold text-[#c5a059] mb-4 uppercase tracking-widest text-center"
                  {...props}
                />
              ),
              ...
              p: ({ node, ...props }) => (
                <p
                  className="text-sm font-light leading-7 text-slate-600 mb-4 text-center"
                  {...props}
                />
              ),
              ...
              ul: ({ node, ...props }) => (
                <ul
                  className="list-none mb-6 space-y-2 text-sm font-light leading-7 text-slate-600 text-center"
                  {...props}
                />
              ),
              ...
              a: ({ node, ...props }) => (
                <a
                  className="text-[#c5a059] hover:text-[#b08d4b] underline decoration-[0.5px] underline-offset-4"
                  {...props}
                />
              ),

              // HR
              hr: ({ node, ...props }) => <hr className="my-10 border-[#e5dcc5]" {...props} />,
            }}
          >
            {bodyMarkdown}
          </ReactMarkdown>
