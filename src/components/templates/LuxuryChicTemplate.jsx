...
const LuxuryChicTemplate = ({ markdown, userProfile }) => {
  ...
  if (userProfile?.location || userProfile?.city)
    contactItems.push({ icon: MapPin, value: userProfile.location || userProfile.city });
  if (userProfile?.linkedinUrl || userProfile?.linkedin)
    contactItems.push({
      icon: Linkedin,
      value: (userProfile.linkedinUrl || userProfile.linkedin).replace(/^https?:\/\/(www\.)?/, ''),
    });
  if (userProfile?.portfolioUrl || userProfile?.website)
    contactItems.push({
      icon: Globe,
      value: (userProfile.portfolioUrl || userProfile.website).replace(/^https?:\/\//, ''),
    });
      ...
      <header className="pt-20 pb-8 px-16 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-8 text-xs font-bold tracking-widest uppercase">
          {contactItems.map((item, i) => (
            <div
              key={i}
              className="flex items-center gap-2 border-b border-transparent hover:border-black transition-all pb-0.5"
            >
      ...
      <div className="px-16 pb-20 max-w-3xl mx-auto">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-2xl font-['Cinzel',serif] text-black mt-8 mb-8 text-center"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
              <h3
                className="text-sm font-['Montserrat',sans-serif] font-bold uppercase tracking-widest text-black mt-10 mb-2 text-center"
                {...props}
              />
            ),
            h4: ({ node, ...props }) => (
              <h4
                className="text-xs font-['Montserrat',sans-serif] font-light italic text-slate-500 mb-6 text-center"
                {...props}
              />
            ),
            p: ({ node, ...props }) => (
              <p
                className="mb-6 text-center font-['Montserrat',sans-serif] font-light text-sm leading-8 tracking-wide text-slate-800"
                {...props}
              />
            ),
            ul: ({ node, ...props }) => (
              <ul
                className="list-none space-y-3 mb-8 text-center font-['Montserrat',sans-serif] font-light text-sm"
                {...props}
              />
            ),
            ...
            a: ({ node, ...props }) => (
              <a
                className="text-black border-b border-black pb-0.5 hover:pb-1 transition-all"
                {...props}
              />
            ),
            hr: ({ node, ...props }) => (
