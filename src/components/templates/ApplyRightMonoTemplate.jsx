...
const ApplyRightMonoTemplate = ({ markdown, userProfile }) => {
  ...
  return (
    ...
    >
      {/* SIDEBAR — no fill, just a divider */}
      <div data-cv-sidebar className="w-[32%] border-r border-slate-200 p-7 shrink-0">
        {userProfile?.photoUrl && (
          <img src={userProfile.photoUrl} alt="" className="w-20 h-20 rounded-full object-cover mx-auto mb-5 border border-slate-200" />
        )}
        {contactItems.length > 0 && (
          <div className="mb-6">
            <h2 className="text-[9pt] font-bold uppercase tracking-[0.14em] text-slate-400 mb-2 pb-1 border-b border-slate-200">Details</h2>
            <div className="space-y-2 text-[9pt] text-slate-600">
        ...
        {skillsMarkdown && (
          <div className="mb-6">
            <h2 className="text-[9pt] font-bold uppercase tracking-[0.14em] text-slate-400 mb-2 pb-1 border-b border-slate-200">Skills</h2>
        ...
        {languagesLine && (
          <div>
            <h2 className="text-[9pt] font-bold uppercase tracking-[0.14em] text-slate-400 mb-2 pb-1 border-b border-slate-200">Languages</h2>
            <p className="text-[9pt] text-slate-600">{languagesLine}</p>
          </div>
        )}
      </div>
      {/* MAIN — name/role header lives here, not in the sidebar */}
      <div className="w-[68%]" style={{ padding: 'var(--cv-margin, 2rem)' }}>
        <header className="mb-6">
          <h1 className="text-[19pt] font-bold uppercase tracking-[-0.01em] text-slate-900" style={{ color: 'var(--cv-accent, #1a1a1a)' }}>{name}</h1>
          {roleTitle && <div className="text-[10pt] text-slate-500 font-medium uppercase tracking-[0.05em] mt-1">{roleTitle}</div>}
        </header>
        <ReactMarkdown components={{
          h1: () => null,
          h2: (p) => <h2 className="text-[11pt] font-bold uppercase tracking-[0.08em] mt-5 mb-2.5 pb-1 border-b border-slate-300 first:mt-0" {...p} />,
