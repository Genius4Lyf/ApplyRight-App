          ...
          <div className="text-right text-xs font-['EB_Garamond',serif] text-[#546e7a] leading-5">
            {contactItems.map((item, i) => (
              <div key={i} className="flex items-center justify-end gap-2">
                ...
                <item.icon size={12} className="text-[#546e7a]" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-12 font-['EB_Garamond',serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-xl font-bold text-[#1a252f] mt-8 mb-4 border-b border-[#d0d0d0] pb-1 uppercase tracking-wider text-xs font-sans"
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-disc pl-5 mb-4 text-[11pt] leading-7 space-y-1 marker:text-[#b0bec5]"
                {...props}
              />
            ),
            ...
            a: ({ node, ...props }) => (
              <a
                className="text-[#34495e] border-b border-[#90a4ae] hover:text-[#1a252f] transition-colors"
                {...props}
              />
            ),
          }}
