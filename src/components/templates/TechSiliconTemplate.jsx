        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-500 font-medium">
            {contactItems.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-2 hover:text-indigo-500 transition-colors cursor-pointer"
              >
                <item.icon size={15} className="text-indigo-500 opacity-80" />
                {item.value}
              </div>
            ))}
          </div>
      ...
      <div className="px-10 pb-12 font-['Inter',sans-serif]">
        <ReactMarkdown
          components={{
            ...
            h2: ({ node, ...props }) => (
              <h2
                className="text-lg font-bold text-slate-900 mt-8 mb-4 flex items-center gap-3"
                {...props}
              >
            ...
            ul: ({ node, ...props }) => (
              <ul
                className="list-none space-y-2 mb-4 text-[10.5pt] leading-7 text-slate-600"
                {...props}
              />
            ),
            li: ({ node, ...props }) => (
              <li
                className="pl-4 relative before:content-[''] before:absolute before:left-0 before:top-[10px] before:w-1.5 before:h-1.5 before:bg-indigo-400 before:rounded-full"
                {...props}
              />
            ),
            strong: ({ node, ...props }) => (
