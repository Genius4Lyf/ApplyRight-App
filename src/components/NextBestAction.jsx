...
const NextBestAction = ({
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="font-semibold text-slate-800 dark:text-slate-200 text-sm mt-0.5 truncate">
              {cvGenStatus.stageMessage || 'Working…'}
            </p>
          </div>
          <span className="text-sm font-bold text-indigo-600 dark:text-indigo-300 shrink-0">
            {progress}%
          </span>
        </div>
        ...
        >
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-bold text-[15px] sm:text-base mt-0.5 leading-snug text-slate-900 dark:text-slate-100">
                {completed.label}
              </p>
              <p className="text-[13px] sm:text-sm mt-1 leading-snug text-slate-600 dark:text-slate-300">
                {completed.subtitle}
              </p>
            </div>
          </div>
          {completed.advisory && (
            <div className="flex items-start gap-2 mb-4 px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 text-amber-800 dark:text-amber-300">
  ...
  return (
    ...
    >
      {isWeakFit && !hasCV && (
        <div className="flex items-start gap-2 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 rounded-lg px-3 py-2">
          <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
          <span>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            className={`text-[13px] sm:text-sm mt-1 leading-snug ${
              action.tone === 'primary' ? 'text-white/85' : 'text-slate-500 dark:text-slate-400'
            }`}
          >
            {action.subtitle}
          </p>
      ...
      {showBundle && (
        <CreditGate cost={CREDIT_COSTS.GENERATE_BUNDLE}>
          ...
          >
            ...
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 shrink-0 self-end sm:self-auto">
              18 cr <span className="text-emerald-600 dark:text-emerald-300">(save 2)</span>
            </span>
          </button>
        </CreditGate>
      )}
