...
const TailorDiffView = ({ originalCVId, currentCVData, tailoredForJob, atsScores }) => {
  ...
  if (loading) {
    return <div className="text-center py-6 text-sm text-slate-400">Loading comparison...</div>;
  }
  ...
  return (
    <div className="space-y-3 animate-in fade-in duration-500">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="font-bold text-slate-800">What Changed</h3>
        {tailoredForJob && (
        ...
        <Section title="Skills" defaultOpen>
          <div className="flex flex-wrap gap-1.5">
            {keptSkills.map((s) => (
              <span
                key={s}
                className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full bg-slate-100 text-slate-600"
              >
            ...
            {addedSkills.map((s) => (
              <span
                key={s}
                className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 font-medium"
              >
            ...
            {removedSkills.map((s) => (
              <span
                key={s}
                className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full bg-red-50 text-red-500 line-through"
              >
