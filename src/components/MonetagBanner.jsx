        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">
              {status === 'verifying' ? (
                <>
                  <strong className="text-orange-400">Keep tab open</strong> for {timeLeft}s
                </>
              ) : status === 'tab-closed' ? (
                'Please keep tab open next time'
              ) : (
                subtitle
              )}
            </p>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        {/* Icon Badge */}
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 transition-colors ${status === 'success' ? 'bg-green-100 text-green-600' : 'bg-indigo-100 text-indigo-600'}`}
        >
        ...
        <p className="text-slate-500 mb-6 flex-grow">
          {status === 'verifying' ? (
            <>
              <strong className="text-orange-600">Keep tab open</strong> for {timeLeft}s...
            </>
          ) : status === 'success' ? (
            '5 A.I Credits have been added!'
          ) : status === 'tab-closed' ? (
            'Tab was closed too early. Try again!'
          ) : (
            <>
              View this offer to instantly earn{' '}
              <span className="font-bold text-slate-900">5 A.I Credits</span>.
            </>
          )}
        </p>
        ...
        {/* Button */}
        <button
          className={`w-full py-3 px-4 font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 ${status === 'success' ? 'bg-green-500 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}
        >
