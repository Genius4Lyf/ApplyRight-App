 ...
 *
 * A full analysis is five cards deep and most of it is detail you want ON DEMAND — read
 * the verdict, then open the part you care about. Rendered end-to-end in a chat column it
 * was a very long scroll where the actions at the bottom were easy to miss entirely.
 *
 * Three things this is careful about:
 *
 *  1. THE SUMMARY LINE IS THE POINT. A collapsed section still says what it found ("4 of
 *     11 matched"), so closing one never costs you the finding — only the detail behind
 *     it. A disclosure that collapses to a bare title just hides information.
 *  2. IT CLOSES FROM THE BOTTOM. After reading a long section the header is off screen,
 *     and scrolling back up to shut what you just read is the exact friction this is
 *     supposed to remove.
 *  3. `height: auto` is animated, not toggled, so an opening section pushes what follows
 *     it rather than snapping the page.
 */
const AnalysisSection = ({
  eyebrow,
  // Rendered INSIDE the eyebrow, beside its text — a help tooltip, usually. It gets its
  // own slot rather than being folded into `eyebrow` as a node, because a nested wrapper
  // there stopped the label matching its siblings: every other section's eyebrow is a
  // direct text child of the styled span, and the one that wasn't rendered visibly larger.
  // One slot, one text path, four identical labels.
  eyebrowAside,
  title,
  summary,
  defaultOpen = false,
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
      ...
      >
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 block font-heading text-xl font-bold text-slate-900 dark:text-slate-100">
            {title}
          </span>
          {!open && summary && (
            <span className="mt-1 block truncate font-mono text-[0.7rem] tabular-nums text-slate-400 dark:text-slate-500">
              {summary}
            </span>
          )}
        </span>
          ...
          >
            ...
            <div className="px-6 pb-4 pt-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-slate-500 transition-colors hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
              >
...
// The verdict on one dimension — "Meets", "Below preferred".
//
// A dash and a colour, not a bordered pill. The pill drew a box around two words sitting
// inside a line of plain text and read as a control you could press; the colour was
// already carrying the whole meaning, so the border was decoration competing with it. The
// separator is aria-hidden — a screen reader gets the value and the verdict as one phrase
// without a dash read out between them.
...
const FitScoreCard = ({ fitScore, fitAnalysis, actionPlan, optimizedFitScore, applicationId }) => {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <AnalysisSection
        eyebrow={t('fitScoreCard.theGap')}
        title={t('fitScoreCard.whereShort')}
        summary={
          totalSkills > 0
      ...
      {Array.isArray(fitAnalysis?.evidence) && fitAnalysis.evidence.length > 0 && (
        <AnalysisSection
          eyebrow={t('fitScoreCard.readFromResume')}
          title={t('fitScoreCard.whatStoodOut')}
          summary={t('fitScoreCard.evidenceCount', { count: fitAnalysis.evidence.length })}
        >
      ...
      {breakdown.skillsScore != null &&
        (() => {
          ...
          return (
            <AnalysisSection
              eyebrow={t('fitScoreCard.theMath')}
              eyebrowAside={
                /* Explains the fixed weights so users understand why, not just what.
                   Hover or focus reveals it. */
                <span className="group relative flex items-center">
                  <HelpCircle
                    // Sized to the eyebrow, not to a default: a 14px icon in an 11px line
                    // stretched this row taller than every other eyebrow.
                    className="h-3 w-3 shrink-0 cursor-help text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
                    tabIndex={0}
                  />
                  <span
                    role="tooltip"
                    className="pointer-events-none invisible absolute bottom-full left-1/2 z-10 mb-2 w-64 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-lg bg-slate-900 px-3 py-2 text-xs normal-case tracking-normal text-white opacity-0 shadow-lg transition-all group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100"
                  >
                    <span className="mb-1 block font-semibold">{t('fitScoreCard.whyWeights')}</span>
                    <span className="block leading-relaxed text-slate-200">
                      {t('fitScoreCard.weightsExplainer')}
                    </span>
                  </span>
                </span>
              }
              title={t('fitScoreCard.scoreBreakdown')}
              summary={dimensions.map((d) => `${d.label} ${d.score}`).join(' · ')}
            >
      ...
      {actionPlan && actionPlan.length > 0 && (
        <AnalysisSection
          eyebrow={t('fitScoreCard.yourMove')}
          title={t('fitScoreCard.whatToFixNext')}
          summary={t('fitScoreCard.actionCount', { count: actionPlan.length })}
          // The only one OPEN by default: it is the part of the analysis that asks the
          // reader to do something, and a to-do list nobody opens is a to-do list nobody
          // does.
          defaultOpen
        >
