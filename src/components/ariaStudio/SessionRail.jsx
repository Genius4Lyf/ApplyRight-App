...
import { Plus, FilePlus2, Trash2, Pencil } from 'lucide-react';
            ...
            {sessions.map((s) => {
              ...
              return (
                <li key={s._id} className="group relative">
                  ...
                  {isRenaming ? (
                    <div className={`pl-4 ${canRename ? 'pr-16' : 'pr-10'} py-2.5`}>
                      <input
                        ref={renameInputRef}
                        type="text"
                        value={renameDraft}
                    ...
                    <button
                      ...
                      className={`w-full text-left pl-4 ${canRename ? 'pr-16' : 'pr-10'} py-2.5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-900 dark:focus-visible:ring-slate-100 ${
                  ...
                  {/* Rename — build sessions only (a tailor row's heading is the job title,
                  not a stored name). Sits left of delete, same reveal-on-hover treatment. */}
                  {canRename && !isRenaming && (
                    <button
                      type="button"
                      onClick={() => beginRename(s, heading)}
                      title={t('ariaStudio.desk.renameCv')}
                      aria-label={t('ariaStudio.desk.renameCv')}
                      className="absolute right-9 top-1.5 flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all md:opacity-0 md:group-hover:opacity-100 md:focus-visible:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
                    >
