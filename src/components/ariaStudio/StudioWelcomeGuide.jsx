          ...
          >
            <div className="bg-white/95 px-5 pb-6 pt-5 dark:bg-slate-900/95 sm:px-7 sm:pb-7 sm:pt-6">
              ...
              <div
                ...
                aria-label={t('ariaStudio.welcomeGuide.progress', {
                  current: step + 1,
                  total: STEPS.length,
                })}
              >
                {STEPS.map((item, index) => (
                  <span
                    key={item.key}
                    className={`h-1 flex-1 rounded-full transition-colors ${index <= step ? 'bg-slate-900 dark:bg-white' : 'bg-slate-100 dark:bg-slate-800'}`}
                  />
                ))}
              </div>
              <div className="mt-7 flex min-h-[242px] flex-col items-start">
                <div className="mb-5 flex items-center gap-3">
                  <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-900 dark:text-white">
                    {t('ariaStudio.welcomeGuide.stepLabel', {
                      current: step + 1,
                      total: STEPS.length,
                    })}
                  </span>
                  <span className="h-px w-7 bg-slate-900/50 dark:bg-white/50" />
                </div>
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    <h2
                      ...
                      className="max-w-md text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 sm:text-[1.7rem]"
                    >
                    ...
                    <div className="mt-5 space-y-2.5 border-l border-slate-200 pl-4 dark:border-slate-700">
                      <p className="flex gap-2 text-[13px] leading-5 text-slate-700 dark:text-slate-200">
                        ...
                        {t(`ariaStudio.welcomeGuide.steps.${current.key}.detailOne`)}
                      </p>
                      <p className="flex gap-2 text-[13px] leading-5 text-slate-700 dark:text-slate-200">
                        <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-900 dark:text-white" />
                        {t(`ariaStudio.welcomeGuide.steps.${current.key}.detailTwo`)}
                      </p>
                    </div>
                    <a
                      href="/aria-studio-guide"
                      className="mt-5 inline-flex items-center gap-1.5 text-[12px] font-semibold text-slate-500 no-underline transition-colors hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
                    >
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-white/95 px-5 py-4 dark:border-slate-800 dark:bg-slate-900/95 sm:px-7">
              {step > 0 ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-1 py-2 text-sm font-semibold text-slate-500 transition-colors hover:text-slate-900 dark:hover:text-slate-100"
                >
