...
const FeedbackPage = () => {
  ...
  const checkUser = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/feedback/check-user`, {
        contactValue,
      });
      if (res.data.success) {
        setUserFound(true);
                ...
                {userFound ? (
                  <form onSubmit={submitFeedback} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`text-xs ${message.length >= 300 ? 'text-red-500 font-medium' : 'text-slate-400'}`}
                        >
                          {message.length}/300
                        </span>
                      </div>
                  ...
                  <div className="text-center py-4">
                    ...
                    <p className="text-slate-500 mb-8 max-w-sm mx-auto">
                      We check for an account to prevent spam and ensure quality feedback. Please
                      sign up to continue.
                    </p>
