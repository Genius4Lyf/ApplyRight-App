  ...
  const handleToggleFeature = async (feedbackId) => {
    try {
      ...
      if (data.success) {
        // Update local state
        setFeedbacks((prev) =>
          prev.map((f) => (f._id === feedbackId ? { ...f, isFeatured: data.data.isFeatured } : f))
        );
    ...
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Insights from your users.</p>
        </div>
            ...
            <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 border-dashed">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {searchTerm
                  ...
                  : 'Wait for users to share their thoughts'}
              </p>
            </div>
          )}
        </>
