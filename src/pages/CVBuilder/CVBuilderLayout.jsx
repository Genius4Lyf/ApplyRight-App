      ...
      >
        <div className="w-full max-w-sm my-auto flex flex-col items-center text-center">
          ...
          <div className="relative mb-4 group">
            <div className="absolute inset-0 rounded-full bg-slate-900/10 dark:bg-white/5 blur-md group-hover:blur-lg transition-all duration-300" />
            <div className="relative w-16 h-16 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-center text-slate-900 dark:text-white">
            ...
            {requiredJourneySteps.map((step, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {!isStepDone && (
                    <span className="text-[9px] font-extrabold text-slate-900 dark:text-white opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-wider flex items-center gap-0.5">
...
const CVBuilderInner = () => {
  ...
  // Your other CVs, one tap away. Drawn OVER the wizard rather than beside it: this
  // page already spends its width on a form and a 440px coach, and a third column would
  // squeeze the form to nothing. Scoped to 'builder' — the CVs this wizard can open.
  const { openSidebar, sidebar } = useWorkspaceSidebar({
    scope: 'builder',
    activeId: cvData?._id,
  });
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative min-h-0">
          ...
          <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-2 md:px-4 py-2 flex items-center gap-2 md:gap-3 shrink-0">
            {/* First thing in the row, on every surface that carries a sidebar — it is
                the only sign on screen that the list exists. */}
            ...
            {saving && (
              <span className="text-xs text-slate-500 dark:text-slate-400 animate-pulse flex items-center gap-1 shrink-0">
      ...
      {!isDesktop && (
        <>
          ...
          >
            ...
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white">
                <AriaOrbit size={21} />
              </span>
