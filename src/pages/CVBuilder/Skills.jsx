...
const Skills = () => {
  ...
  // Generated-skills review modal payload ({ suggestions, bestForRole, isPaid }).
  // null = closed. Replaces the old auto-merge so the user curates before adding.
  ...
  if (!cvData) {
    return (
      <div className="p-8 text-center text-slate-500 dark:text-slate-400">Loading skills...</div>
    );
  }
    ...
    try {
      ...
      if (result.suggestions?.length) {
        setGenModal({
          ...
          bestForRole: result.bestForRole || [],
          isPaid: result.isPaid ?? isPro,
        });
        setShowAiBanner(false); // Hide the auto-fill banner after a successful run
        // Keep the credit counter fresh (skipped on a cache hit / paid tier).
        if (result.charged && typeof result.remainingCredits === 'number') {
          window.dispatchEvent(
            new CustomEvent('credit_updated', { detail: result.remainingCredits })
          );
        }
      } else {
        toast.error('No skills generated — add more detail to your profile and try again.');
      }
    } catch (error) {
      console.error(error);
  ...
  // Merge the skills the user selected in the review modal (deduped, metadata
  // preserved for paid users so the ⓘ popover + Save-to-Prep still work).
  const handleAddFromModal = (picked) => {
    const additions = (picked || []).filter(
      (p) => !skillsData.some((ex) => ex.name.toLowerCase() === p.name.toLowerCase())
    );
    setGenModal(null);
    if (additions.length === 0) {
      toast.info('Those skills are already on your CV.');
      return;
    }
    setSkillsData([...skillsData, ...additions]);
    setStepDirty?.(true);
    ...
    >
            ...
            <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-xl p-3 sm:px-4 sm:py-3 relative">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 pr-8 sm:pr-10">
                ...
                {isPro ? (
                  ...
                  <CreditGate cost={CREDIT_COSTS.GENERATE_SKILLS}>
                    <button
                      type="button"
                      onClick={handleGenerateSkills}
                      disabled={loadingAi}
      ...
      <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
        ...
        <p className="text-[11px] text-slate-400 dark:text-slate-500 px-1">
          ...
          to add. Pick a category or create a new one.
        </p>
      </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">
                  {isPro
                    ? `These ${skillsWithEvidenceCount} skill${skillsWithEvidenceCount === 1 ? '' : 's'} come with evidence from your work, education, and projects, plus talking points you can rehearse. These are exactly what interviewers tend to drill into — save them so you can review before the call.`
                    : 'Export your auto-generated skills directly to your Mock Interview Prep bank along with pre-written STAR-method answers and work history evidence maps. Upgrade to Pro to enable.'}
                </p>
      ...
      {/* Generated-skills review modal — opened after a successful Auto-fill. */}
      {genModal && (
        <GeneratedSkillsModal
          suggestions={genModal.suggestions}
          bestForRole={genModal.bestForRole}
          isPaid={genModal.isPaid}
          existingSkills={skillsData.map((s) => s.name)}
          hasJD={!!cvData.targetJob?.description}
