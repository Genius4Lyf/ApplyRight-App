...
// Uppercase alias keeps the portaled motion element identifiable as a React component.
const Motion = motion;
...
const TargetJob = () => {
  ...
  const chooseCvDirection = (choice) => {
    ...
    if (choice === 'tailor') {
      openTargetJobForm();
      choiceTimer.current = setTimeout(() => setChoiceTransition(''), reduceMotion ? 180 : 900);
      return;
    }
    choiceTimer.current = setTimeout(() => proceed(), reduceMotion ? 180 : 2200);
  };
                      ...
                      <div className="mt-3">
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                          {mustHaves.map((item) => (
                            <span
                              key={item}
                              className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200"
                            >
                              {item}
                            </span>
                          ))}
      ...
      {createPortal(
        <Motion.div
          ...
          animate={{
            ...
            top: orbitTarget.top,
            scale:
              transitioningStage && !targetWorkspace ? orbitTarget.scale * 1.28 : orbitTarget.scale,
            opacity: orbitTarget.ready && !ariaDrawerOpen ? 1 : 0,
          }}
          transition={
            reduceMotion
              ? { duration: 0.1 }
